...
the index will be tracked solely by the API
for that `struct`.
...
//
Indeed, Flux _cannot_ check this code, because it knows nothing
about the `inner` field of the `RVec`, as it is opaque.
//
...
fn test_push() -> RVec<i32> {
    ...
    vec.push(1);               // vec: RVec<i32>[1]
    vec.push(2);               // vec: RVec<i32>[2]
...
Aha, but what if the vector is empty? You _could_ return an `Option<T>`, and
...
```rust
error[0999]: precondition might not hold
   |
24 |     v.pop();
   |     ^^^^^^^ call site
   |
...
Its impolite to force the caller to only pass in non-empty
vectors to `pop`, without providing them way to determine
_whether_ the vector is non-empty.
//
That is, our API should allow us to _peek_ at the size of the vector
...
verifies, i.e. so that Flux knows that after two `push`es, the value
returned by `.len()` is exactly `2`, and after two `pop`s the size is
back to `0`.
])
...
Of course, vectors are not just stacks. Vectors also
allow random access to their elements which is precisely
where those pesky panics occur, and where the refined
vector API gets rather useful.
//
...
We just saw how Flux's index and constraint
mechanisms work with Rust's ownership to let
us write a _refined vector API_ that ensures
the safety of all vector accesses at compile time.
//
